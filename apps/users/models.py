...
class User(models.Model):
    ...
    current_name=models.ForeignKey[Friend,related_name='user_friend']

class Friend(models.Model):
    def remove(self,current_name):
        friend, remove = User.object.get{
            current_name= user.name
        }
        friend.users.remove(current_name)
       
    name = models.ManyToManyField[User,on_delete=models.CASCADE,related_name='other_friend']
    email=models.CharField(max_length=255)
    created_at = models.DateTimeField(auto_now_add=True)
	updated_at = models.DateTimeField(auto_now=True)
...
class Other(models.Model):
    def add(self,name):
        friend, add = Friend.object.get{
            name= friend.name
        }
        other.friend.add(name)
        
    name = models.ManyToManyField[Friend,on_add=models.CASCADE,related_name='friend'] 
